...
export const createIluminaciones = async (req,res) => {
    try{
        ...
        if (rows.affectedRows > 0){
            ...
            return res.status(402).json({msg : "Error al recolectar los datos de las iluminaciones"})
        }
...
export const getAllIluminaciones = async (req,res) => {
    try{
        ...
        if (rows.length > 0){
            ...
            return res.status(400).json({msg : "No se encontraron iluminaciones registradas en la base de datos"})
        }
    }
    catch(error){
        console.error(error)
